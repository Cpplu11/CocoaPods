...
module Pod
  class Installer
    ...
    class Analyzer
      ...
      #
      # @param  [Hash{Podfile::TargetDefinition => Array<ResolverSpecification>}] resolver_specs_by_target
      ...
      def generate_targets(resolver_specs_by_target, target_inspections)
        ...
        aggregate_targets = resolver_specs_by_target.keys.reject(&:abstract?).map do |target_definition|
          generate_aggregate_target(target_definition, target_inspections, pod_targets_by_target_definition, resolver_specs_by_target)
        end
        aggregate_targets.each do |target|
      ...
      #
      # @param  [Hash{Podfile::TargetDefinition => Array<ResolverSpecification>}] resolver_specs_by_target
      #         mapping of targets to resolved specs
      #
      # @return [AggregateTarget]
      #
      def generate_aggregate_target(target_definition, target_inspections, pod_targets_by_target_definition, resolver_specs_by_target)
        if installation_options.integrate_targets?
        ...
        pod_targets_for_build_configuration = filter_pod_targets_for_target_definition(target_definition, pod_targets_by_target_definition,
                                                                                       resolver_specs_by_target, build_configurations)
      ...
      #
      def filter_pod_targets_for_target_definition(target_definition, pod_targets_by_target_definition, resolver_specs_by_target, build_configurations)
        ...
        pod_targets_by_target_definition[target_definition].each do |pod_target|
          ...
          resolver_spec = resolver_specs_by_target[target_definition].find do |spec|
          ...
          build_configurations.each do |configuration_name|
            ...
            if resolver_spec.transitive?
              ...
              whitelists += resolver_spec.incoming_vertices.map do |incoming_spec|
                target_definition.pod_whitelisted_for_configuration?(incoming_spec.name, configuration_name)
              end
              whitelists.uniq!
            end
